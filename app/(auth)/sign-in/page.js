...
export default function SignInPage() {
  ...
  const recaptchaId = "signin-recaptcha-container"
  const recaptchaRef = useRef(null)

  useEffect(() => {
    if (!recaptchaRef.current) {
      recaptchaRef.current = getRecaptchaVerifier(recaptchaId)
    }
  ...
  async function onSendOtp(e) {
    ...
    try {
      ...
      if (err.code === "auth/invalid-phone-number") {
        ...
        errorMessage = "Authentication service unavailable. Please refresh and try again.";
      } else if (err.code === "auth/app-not-authorized") {
        errorMessage = "Authentication service not authorized. Please contact support.";
  ...
  async function onVerifyOtp(e) {
    ...
    try {
      ...
      const res = await confirmation.confirm(otp.join(""))
      const idToken = await res.user.getIdToken()

      const r = await fetch("/api/auth/phone/signin", {
        method: "POST",
  ...
  // Handle OTP input with better UX
